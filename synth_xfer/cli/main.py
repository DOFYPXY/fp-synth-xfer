...
from synth_xfer._util.solution_set import (
    SolutionSet,
    UnsizedSolutionSet,
)
...
def get_concrete_function(concrete_op_name: str, extra: int | None) -> FuncOp:
    ...
    for k in comb_semantics.keys():
        if k.name == concrete_op_name:
            ...
            if concrete_op_name == "comb.mux":
                ...
                funcTy = FunctionType.from_lists([intTy, intTy], [IntegerType(width * 2)])
                result = FuncOp(func_name, funcTy)
    ...
    assert result is not None and (f"Cannot find the concrete function for {concrete_op_name}")
    return result
...
def solution_set_eval_func(
    ...
    return lambda transfer=list[FunctionWithCondition], base=list[FunctionWithCondition]: (
        eval_transfer_func_helper(data_dir, transfer, base, ret_top_func, domain, helper_funcs)
    )
...
def solution_set_tests_sampler(
    ...
    return lambda base=list[FunctionWithCondition], samples=int, seed=int: tests_sampler_helper(
...
def run(
    ...
    EvalResult.init_bw_settings(set(lbws), set([t[0] for t in mbws]), set([t[0] for t in hbws]))
    ...
    data_dir = setup_eval(domain, lbws, mbws, hbws, random_seed, "\n".join(helper_funcs_cpp))

    solution_eval_func = solution_set_eval_func(data_dir, domain, helper_funcs_cpp, ret_top_func)
    ...
    for ith_iter in range(num_iters):
        ...
        current_total_rounds += (total_rounds - current_total_rounds) // (num_iters - ith_iter)
        current_num_abd_procs += (num_abd_procs - current_num_abd_procs) // (num_iters - ith_iter)
        ...
        hbw_log = "\n".join(
            f"bw: {res.bitwidth}, dist: {res.dist:.2f}" for res in final_cmp_res[0].get_high_res()
        )
        logger.info(
