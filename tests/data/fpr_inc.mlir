"func.func"() ({
  ^0(%a : !fp.abs_value):
    ...
    %one = "fp.constant"(%a_lo) {value = 1.0} : (!fp.float) -> !fp.float
    %sum_lo = "fp.add"(%a_lo, %one) : (!fp.float, !fp.float) -> !fp.float
    %sum_hi = "fp.add"(%a_hi, %one) : (!fp.float, !fp.float) -> !fp.float
    ...
    %res_has_nan = "arith.ori"(%a_has_nan, %sum_has_nan) : (i1, i1) -> i1
  ...
  }) {"sym_name" = "fpr_inc", "function_type" = (!fp.abs_value) -> !fp.abs_value} : () -> ()
