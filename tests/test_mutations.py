...
# Helper functions
def make_context(
    seed: int = 42, domain: AbstractDomain | None = None
) -> SynthesizerContext:
    random = Random(seed)
...
def assert_program_valid(prog: MutationProgram):
    ...
    assert isinstance(ops[-1], ReturnOp), f"Last op must be ReturnOp, got {type(ops[-1])}"
    ...
    for operand in make_op.operands:
        assert operand in defined, "MakeOp uses undefined value"
...
class TestMutationProgramPrimitives:
    ...
    def test_subst_operation_preserves_validity(self):
        ...
        valid_operands = {
            ty: spl.current.get_valid_operands(idx, ty) for ty in [INT_T, BOOL_T]
        }
        new_op = None
    ...
    def test_revert_operation_restores_program(self):
        ...
        spl.replace_entire_operation(
            spl.current.get_modifiable_operations()[0][1], history=True
        )
...
class TestSampleNext:
    ...
    def test_replace_entire_operation_changes_op_type(self):
        ...
        for _ in range(20):
            ...
            assert_program_valid(spl.current)
            if not isinstance(spl.current.ops[idx], original_type):
                changed = True
            spl.current.revert_operation()
...
class TestPerturbConstant:
    ...
    def test_perturb_constant_revert_restores_value(self):
        ...
        def get_const_values():
            return [
                op.value.value.data for op in spl.current.ops if isinstance(op, Constant)
            ]
    ...
    def test_perturb_constant_kb_samples_valid_values(self):
        ...
        spl = make_sampler(flags=flags, seed=0, domain=AbstractDomain.KnownBits)
        for _ in range(30):
            spl.sample_next()
    ...
    def test_perturb_constant_cr_samples_valid_values(self):
        ...
        spl = make_sampler(flags=flags, seed=0, domain=AbstractDomain.UConstRange)
        for _ in range(30):
            spl.sample_next()
            for op in spl.current.ops:
                if isinstance(op, Constant):
    ...
    def test_perturb_constant_always_changes_value(self):
        ...
        for _ in range(30):
            ...
            after = get_const_values()
            if spl.current.old_op is not None:
                assert before != after, "perturb_constant should always change the value"
                spl.current.revert_operation()
    ...
    def test_perturb_constant_fp_samples_valid_values(self):
        ...
        spl = make_sampler(flags=flags, seed=0, domain=AbstractDomain.FPRange)
        for _ in range(30):
            spl.sample_next()
            for op in spl.current.ops:
                if isinstance(op, Constant):
...
class TestReplaceOpWindow:
    ...
    def test_window_changes_multiple_ops(self):
        ...
        assert multi_op_count > 0, "replace_op_window was never triggered in 20 steps"
